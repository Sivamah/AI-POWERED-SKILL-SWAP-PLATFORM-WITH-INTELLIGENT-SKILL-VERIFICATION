        ...
        <div className="max-w-7xl mx-auto px-4 py-8 space-y-12 w-full">
            ...
            <div className="text-center space-y-6 max-w-4xl mx-auto py-8 animate-slide-up">
                <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-white drop-shadow-2xl">
                    Find Your <span className="bg-gradient-to-r from-blue-400 via-cyan-300 to-blue-400 bg-clip-text text-transparent">Mentor</span>
                </h1>
            ...
            <form onSubmit={handleSearch} className="max-w-2xl mx-auto relative group z-20 animate-slide-up delay-100">
                <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/30 via-cyan-500/30 to-blue-500/30 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-700"></div>
                <div className="relative flex items-center bg-slate-900/60 backdrop-blur-xl border border-blue-500/20 rounded-xl p-2 shadow-2xl shadow-blue-500/10">
                    <svg className="w-6 h-6 text-blue-300 ml-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    <input
                        type="text"
                    ...
                    />
                    <button type="submit" className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold px-8 py-3 rounded-lg shadow-lg shadow-blue-500/30 transition-all hover:shadow-blue-500/50">
                        {loading ? (
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                {Array.isArray(results) && results.map((r, idx) => (
                    <div
                        key={r.user_id}
                        className="bg-slate-900/70 backdrop-blur-sm border border-blue-500/20 rounded-2xl shadow-xl shadow-blue-500/5 hover:shadow-blue-500/20 hover:border-blue-500/40 transition-all duration-500 flex flex-col group relative overflow-hidden cursor-pointer"
                        style={{ animationDelay: `${idx * 100}ms` }}
                        onClick={() => setDetailModal({ open: true, mentor: r })}
                    >
                        {/* Soft Teal Glow on Hover */}
                        <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500/0 via-cyan-500/0 to-blue-500/0 group-hover:from-blue-500/20 group-hover:via-cyan-500/20 group-hover:to-blue-500/20 rounded-2xl blur transition duration-500"></div>
                        <div className="relative p-8 flex flex-col h-full gap-6">
                            ...
                            <div className="flex justify-between items-start gap-4">
                                <div className="flex gap-5 items-center">
                                    <div className="relative">
                                        ...
                                        <div className="relative w-16 h-16 rounded-2xl bg-slate-800/80 border border-blue-500/30 flex items-center justify-center text-blue-300 text-2xl font-black shadow-lg">
                                            {r.name.charAt(0)}
                                        </div>
                                    </div>
                                    <div>
                                        <h3 className="text-2xl font-bold text-white tracking-tight group-hover:text-blue-300 transition-colors">{r.name}</h3>
                                        <div className="flex items-center gap-3 mt-1">
                                            <div className="flex gap-0.5">
                                                {[...Array(5)].map((_, i) => (
                                                    <svg key={i} className={`w-3.5 h-3.5 ${i < Math.round(r.reputation || 0) ? 'text-blue-400 fill-cyan-400' : 'text-slate-800'}`} viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.603.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                                                ))}
                                            </div>
                                ...
                                <div className="flex flex-col items-end">
                                    <div className="px-3 py-1 bg-blue-500/15 border border-blue-500/30 rounded-full flex items-center gap-2">
                                        <div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse"></div>
                                        <span className="text-blue-300 font-bold text-[10px] uppercase tracking-widest">{((r.match_score || 0) * 100).toFixed(0)}% MATCH</span>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <div className="bg-white/5 hover:bg-white/10 border border-cyan-500/20 pl-3 pr-2 py-1.5 rounded-lg flex items-center gap-2 transition-all">
                                                ...
                                                {badge && (
                                                    <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded
                                                        ${badge === 'Expert' ? 'bg-blue-500/30 text-blue-300' :
                                                            badge === 'Intermediate' ? 'bg-cyan-500/30 text-cyan-300' :
                            ...
                            <button
                                ...
                                className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold w-full py-3 rounded-xl shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 transition-all flex items-center justify-center gap-2 group/btn"
                            >
                    ...
                    <div className="mentor-card max-w-2xl w-full max-h-[90vh] overflow-y-auto scrollbar-thin" onClick={(e) => e.stopPropagation()}>
                        <div className="relative p-8 space-y-6 bg-gradient-to-br from-slate-900 to-slate-800">
                            ...
                            <div className="flex flex-col items-center text-center space-y-4">
                                {/* Large Avatar */}
                                <div className="relative">
                                    <div className="absolute inset-0 bg-gradient-to-br from-cyan-400 via-lime-400 to-sky-400 rounded-full blur-2xl opacity-50"></div>
                                    ...
                                    {detailModal.mentor.verified && (
                                        <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center border-4 border-slate-900 shadow-lg shadow-blue-500/50">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center justify-center gap-3 mt-2">
                                        ...
                                        {detailModal.mentor.verified && (
                                            <span className="text-xs font-bold uppercase px-3 py-1 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30">
                                                Verified Mentor
                                            </span>
                                        )}
                                    </div>
                                ...
                                <div className="flex items-center gap-6">
                                    <div className="flex items-center gap-2">
                                        <div className="flex gap-0.5">
                                            {[...Array(5)].map((_, i) => (
                                                <svg key={i} className={`w-5 h-5 ${i < Math.round(detailModal.mentor.reputation || 0) ? 'text-blue-400 fill-cyan-400' : 'text-slate-700'}`} viewBox="0 0 20 20" fill="currentColor">
                            ...
                            {detailModal.mentor.experience_level && (
                                <div className="flex justify-center">
                                    <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-wider
                                        ${detailModal.mentor.experience_level === 'Expert' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
            ...
            {requestModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in p-4">
                    <div className="glass-card max-w-sm w-full p-6 space-y-6 relative border-cyan-500/20 shadow-2xl shadow-cyan-500/20">
                        <button
                        ...
                        <div className="text-center space-y-1">
                            ...
                            <p className="text-sm text-slate-400">Requesting <span className="text-cyan-400 font-semibold">{requestModal.skillName}</span> with {requestModal.teacherName}</p>
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            <button
                                onClick={() => confirmRequest('standard')}
                                className="col-span-2 group p-4 rounded-xl border border-white/5 bg-white/5 hover:bg-white/10 hover:border-cyan-500/50 transition-all text-center space-y-3"
                            >
                                <div className="w-10 h-10 mx-auto bg-cyan-500/20 rounded-full flex items-center justify-center text-cyan-400 group-hover:scale-110 transition-transform">
