            ...
            <div className="w-full max-w-4xl bg-[#0f172a] rounded-xl border border-white/10 shadow-2xl flex flex-col relative overflow-hidden min-h-[600px]">
                ...
                <div className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-[#0f172a]/50 backdrop-blur-md z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="font-bold text-lg tracking-tight">Skill Verification: <span className="text-blue-400">{skill}</span></h2>
                    </div>
                    ...
                    {step === 1 && (
                        <div className="flex items-center gap-4 bg-black/30 px-4 py-1.5 rounded-full border border-white/5">
                            <div className="flex items-center gap-2 text-slate-400 text-sm">
                                ...
                                <span className={timeLeft < 60 ? "text-red-400 font-mono" : "text-blue-400 font-mono"}>
                                    {formatTime(timeLeft)}
                                </span>
                            </div>
                        </div>
                ...
                <div className="flex-1 overflow-hidden relative p-8 flex flex-col justify-center">
                    <AnimatePresence mode="wait">
                        ...
                        {step === 0 && (
                            ...
                            >
                                <div className="w-20 h-20 bg-blue-500/20 rounded-2xl flex items-center justify-center mb-6 ring-1 ring-blue-500/50 shadow-[0_0_30px_rgba(168,85,247,0.2)]">
                                    <svg className="w-10 h-10 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 w-full mb-8">
                                    <div className="p-4 rounded-xl bg-white/5 border border-white/5 text-left">
                                        <div className="text-blue-400 font-bold mb-1">Time Limit</div>
                                        <div className="text-sm text-slate-500">10 Minutes</div>
                                    </div>
                                    <div className="p-4 rounded-xl bg-white/5 border border-white/5 text-left">
                                        <div className="text-blue-400 font-bold mb-1">Passing Score</div>
                                ...
                                <button
                                    onClick={() => setStep(1)}
                                    disabled={loading}
                                    className="px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold shadow-lg shadow-blue-500/25 transition-all hover:scale-[1.02] active:scale-[0.98] w-full sm:w-auto"
                                >
                        ...
                        {step === 2 && result && (
                            ...
                            >
                                <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 relative ${result.verified ? 'bg-blue-500/20' : 'bg-red-500/20'}`}>
                                    <div className={`absolute inset-0 border-2 rounded-full animate-ping-slow ${result.verified ? 'border-blue-500/30' : 'border-red-500/30'}`}></div>
                                    {result.verified ? (
                                ...
                                <div className="grid grid-cols-3 gap-4 w-full max-w-lg mb-8">
                                    ...
                                    <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                                        <div className={`text-2xl font-bold mb-1 ${result.percentage >= 70 ? 'text-blue-400' : 'text-red-400'}`}>{Math.round(result.percentage)}%</div>
                                        <div className="text-xs text-slate-500 uppercase font-bold">Percentage</div>
                                    </div>
                                    <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                                        <div className="text-2xl font-bold text-blue-400 mb-1">{result.badge || "-"}</div>
                                ...
                                {!result.verified ? (
                                    ...
                                    <button onClick={onClose} className="px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold shadow-lg shadow-blue-500/25 transition-colors">
